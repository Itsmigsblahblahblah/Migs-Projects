...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const [isAdminConfigureOpen, setIsAdminConfigureOpen] = useState(false);
  ...
  // Reset admin configure menu when mobile menu is closed
  useEffect(() => {
    if (!isMobileMenuOpen) {
      setIsAdminConfigureOpen(false);
    }
  }, [isMobileMenuOpen]);
  ...
  useEffect(() => {
    if (!weatherLoading && isFarmer && weatherAlerts) {
      ...
      const unreadCount = weatherAlerts.filter((alert: any) => {
        ...
        const weatherAlertId = createWeatherAlertId(
          alert.description,
          alert.date || "",
        );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              anytime to access your dashboard.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
      ...
      <header className="sticky top-0 z-50 bg-card/80 backdrop-blur-sm border-b border-border shadow-soft">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              <div
                className={`p-2 rounded-lg ${isAdmin ? "bg-blue-600" : "bg-gradient-primary"}`}
              >
                <Sprout
                  className={`h-6 w-6 ${isAdmin ? "text-white" : "text-primary-foreground"}`}
                />
              </div>
              <div>
                <h1
                  className={`text-lg font-bold ${isAdmin ? "text-blue-600" : "text-primary"}`}
                >
                  Harvestify
                </h1>
              </div>
            ...
            {isFarmer && (
              <nav className="hidden md:flex items-center space-x-4">
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-yellow-500 transition-all duration-300 ${isActive("/prescribe-crop") ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-yellow-500 transition-all duration-300 ${isActive("/crop-history") ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-yellow-500 transition-all duration-300 ${isActive("/market-demand") ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-yellow-500 transition-all duration-300 ${isActive("/history") ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
              </nav>
            )}
            ...
            {isAdmin && (
              <nav className="hidden md:flex items-center space-x-4">
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-blue-600 transition-all duration-300 ${location.hash === "#farmers" ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-blue-600 transition-all duration-300 ${location.hash === "#deletion-requests" ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  <span className="relative z-10">Reports</span>
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-blue-600 transition-all duration-300 ${location.hash === "#reports" ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-blue-600 transition-all duration-300 ${location.hash === "#announcements" ? "w-full" : "w-0 group-hover:w-full"}`}
                  ></span>
                </button>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span
                        className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 bg-blue-600 transition-all duration-300 ${location.pathname === "/admin/crop-prescription" || location.pathname === "/admin/market-demand" ? "w-full" : "w-0 group-hover:w-full"}`}
                      ></span>
                    </button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-56 border-blue-100">
                    <DropdownMenuItem
                      onClick={() => {
                        console.log('Navigating to Soil pH Level');
                        navigate('/admin/crop-prescription');
                      }}
                      className="focus:bg-blue-50 focus:text-blue-700 hover:bg-blue-50 hover:text-blue-700 cursor-pointer transition-colors duration-200"
                    >
                      <TestTubes className="h-4 w-4 mr-2" />
                      <span className="font-medium text-sm">Soil pH Level</span>
                    </DropdownMenuItem>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className={`rounded-full ${isAdmin ? "hover:bg-blue-50" : "hover:bg-yellow-500/20 hover:text-yellow-500"} transition-colors duration-200`}
                    >
                      <User
                        className={`h-5 w-5 ${isAdmin ? "text-blue-600" : ""}`}
                      />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent
                    align="end"
                    className={`w-56 ${isAdmin ? "border-blue-100" : ""}`}
                  >
                    <DropdownMenuLabel>
                      <div className="flex flex-col">
                        <span className={isAdmin ? "text-blue-900" : ""}>
                          {username}
                        </span>
                        <span
                          className={`text-xs ${isAdmin ? "text-blue-700" : "text-muted-foreground"}`}
                        >
        ...
        {isAdmin && isMobileMenuOpen && (
          <div className="md:hidden bg-card border-b border-border">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <div>
                <button
                  onClick={() => setIsAdminConfigureOpen(!isAdminConfigureOpen)}
                  className={`flex items-center justify-between w-full text-left px-3 py-2 text-base font-medium transition-all duration-300 relative hover:text-blue-600 ${isActive("/admin/crop-prescription") || isActive("/admin/market-demand") ? "text-blue-600" : "text-foreground"}`}
                >
                  <span>Configure</span>
                  <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${isAdminConfigureOpen ? "rotate-180" : ""}`} />
                </button>
                <div className={`overflow-hidden transition-all duration-300 ${isAdminConfigureOpen ? "max-h-96 opacity-100 mt-1 mb-2" : "max-h-0 opacity-0"}`}>
                  <div className="pl-4 space-y-1 border-l-2 border-blue-100 ml-3">
                    <button
                      ...
                      className={`flex items-center w-full text-left px-4 py-2 text-sm font-medium rounded-md transition-all duration-300 hover:bg-blue-50 hover:text-blue-600 ${isActive("/admin/crop-prescription") ? "bg-blue-50 text-blue-600" : "text-muted-foreground"}`}
                    >
                      <TestTubes className="h-4 w-4 mr-2" />
                    ...
                    <button
                      ...
                      className={`flex items-center w-full text-left px-4 py-2 text-sm font-medium rounded-md transition-all duration-300 hover:bg-blue-50 hover:text-blue-600 ${isActive("/admin/market-demand") ? "bg-blue-50 text-blue-600" : "text-muted-foreground"}`}
                    >
                      <TrendingUp className="h-4 w-4 mr-2" />
