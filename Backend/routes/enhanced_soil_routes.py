...
try:
    model.load_model('models/enhanced_soil_crop_transformer.keras', 'models/enhanced_soil_preprocessing_pipeline.pkl')
...
async def root():
    return {"message": "Enhanced Soil Crop Recommendation API", 
            "description": "POST /enhanced-recommend to get enhanced crop recommendations based on soil, weather, and market data"}
...
async def health_check():
    logger.info(f"Health check - model: {model}, model.model: {getattr(model, 'model', None)}")
