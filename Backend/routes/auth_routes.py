...
class SendOTPRequest(BaseModel):
    phone_number: str = Field(..., description="Philippine mobile number (09xxxxxxxxx or +639xxxxxxxxx)")
...
async def send_otp(request: SendOTPRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        is_valid, result = validate_philippine_phone_number(request.phone_number)
        if not is_valid:
        ...
        success, message, message_id = otp_service.send_sms_via_semaphore(formatted_number, otp)
        ...
        if success:
            ...
            logger.error(f"Failed to send OTP to {formatted_number}: {message}")
            return SendOTPResponse(
                success=False,
...
async def verify_otp(request: VerifyOTPRequest):
    ...
    try:
        ...
        # Verify OTP
        is_valid, message = otp_service.verify_stored_otp(formatted_number, request.otp)
        ...
        if is_valid:
            ...
            logger.warning(f"OTP verification failed for {formatted_number}: {message}")
...
async def resend_otp(request: ResendOTPRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"Failed to resend OTP to {formatted_number}: {message}")
            return ResendOTPResponse(
                success=False,
