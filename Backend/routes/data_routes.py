...
async def get_crop_data(crop_name: str):
    ...
    try:
        ...
        soil_matches = soil_df[soil_df['Crop'].str.contains(crop_name, case=False, na=False)]
        ...
        veg_matches = veg_df[veg_df['Vegetable'].str.contains(crop_name, case=False, na=False)]
        
        # Load seed price data
        seed_df = pd.read_csv(seed_file)
        seed_matches = seed_df[seed_df['Gulay (Vegetable)'].str.contains(crop_name, case=False, na=False)]
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching crop data: {str(e)}")