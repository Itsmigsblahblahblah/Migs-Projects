...
class OTPService:
    ...
    SEMAPHORE_API_URL = "https://api.semaphore.co/api/v4/otp"
    
    def __init__(self):
        """Initialize the OTP service"""
        self.api_key = os.getenv('SEMAPHORE_API_KEY')
        if not self.api_key:
            logger.warning("SEMAPHORE_API_KEY not found in environment variables")
    ...
    def send_sms_via_semaphore(self, phone_number: str, otp: str) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            # Prepare request - use Semaphore's OTP endpoint
            # Let Semaphore generate its own OTP to avoid dual codes
            payload = {
                'apikey': self.api_key,
                'number': phone_number,
            ...
            if response.status_code == 200:
                    ...
                    if isinstance(response_data, list):
                        ...
                        if response_data and len(response_data) > 0:
                            ...
                            if isinstance(first_item, dict):
                                ...
                                print(f"[SEMAPHORE] List item is not a dict: {first_item}")
                        else:
                        ...
                        print(f"[SEMAPHORE] Unexpected response type: {type(response_data)}")
                    
                    print(f"[SEMAPHORE] SUCCESS - Message ID: {message_id}, Status: {status}")
                    logger.info(f"SMS sent to {phone_number}, status: {status}, message_id: {message_id}")
                ...
                logger.error(f"Failed to send SMS: {error_msg}, status: {response.status_code}")
